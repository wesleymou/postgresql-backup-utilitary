...
(async function () {
  try {
    ...
    if (gcp) {
      ...
      await uploadToGCloudStorage(dumpFilename, databaseContentCompressed, createBucket);
    }
...
async function uploadToGCloudStorage(filename: string, data: string | Buffer, createBucket: boolean) {
  const envParams = {
  ...
  if (!buckets.some(({ name }) => name === envParams.GOOGLE_CLOUD_BUCKET_NAME))
